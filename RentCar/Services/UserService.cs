...
namespace RentCar.Services
{
    public class UserService : IUserService
    {
        ...
        public async Task<ServiceResponse<UserDTO>> GetUserAsync(ClaimsPrincipal user)
        {
            ...
            var userInf = await _context.users.Include(x => x.Messages).Include(x => x.FavoriteCars).Include(x => x.Cars).Include(x => x.Purchases).FirstOrDefaultAsync(x => x.Id == userId);
